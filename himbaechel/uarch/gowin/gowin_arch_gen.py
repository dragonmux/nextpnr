        ...
        for group, arc in groups.items():
            if group == "lut":
                ...
                for name in ["X0", "X2", "X8"]:
                    tmg.set_pip_class(speed, name, group_to_timingvalue(arc[name]), group_to_timingvalue(groups["fanout"][f"{name}Fan"]), TimingValue(1))
                ...
                for name in ["LUT_IN", "DI", "SEL", "CIN", "COUT", "VCC", "VSS", "LW_TAP", "LW_TAP_0", "LW_BRANCH", "LW_SPAN", "GCLK_TAP", "GCLK_BRANCH"]:
                    tmg.set_pip_class(speed, name, TimingValue(100))
                # wires with presently-unknown delay, but known fanout delay (don't ask)
                for name, mapping in [("LUT_OUT", "FFan"), ("FF_OUT", "QFan"), ("OF", "OFFan")]:
                    tmg.set_pip_class(speed, name, TimingValue(100), group_to_timingvalue(groups["fanout"][mapping]))
